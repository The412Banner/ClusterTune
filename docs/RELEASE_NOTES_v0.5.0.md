...
everything from the fork so far plus the new device-info card.
...
build, and a per-cluster cores + max-frequency breakdown. The expanded/collapsed
state is remembered. Everything derives from the device's `Build` info and the
detected CPU policies; SG8350P maps to its **Snapdragon G3 Gen 3** marketing name
(the lookup is easy to extend for more SoCs).
...
- Version bumped to 0.5.0 (versionCode 500).
